...
<body>
	...
	<a href="{{ route('announcement.tambah-data') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Pengumuman Baru</a>
		...
		<tr>
			...
			<td>
				<form onsubmit="return confirm('Apakah Anda Yakin Menghapus Data ini ?');" action="#" method="POST">
					<a href="{{ Route('announcement.edit', $d->id) }}" class="btn btn-sm btn-primary shadow"><i class="fa fa-edit"></i> Edit</a>
					|
					@csrf
					@method('DELETE')
					<button type="submit" class="btn btn-sm btn-danger shadow"><i class="fa fa-trash"></i> Delete</button>
					|
					<a href="{{ Route('announcement.show', $d->id) }}" class="btn btn-sm btn-secondary shadow"><i class="fa fa-info-circle"></i> Detail</a>
				</form>
			</td>
		</tr>
		@php
